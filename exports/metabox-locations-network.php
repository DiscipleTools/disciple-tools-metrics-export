    ...
    public function load_mapping_meta_box() {
        ...
        if ( ! get_post_meta( $post_id, 'gn_geonameid', true ) ) {
            ...
            if ( is_wp_error( $suggestions ) ) {
                echo 'Error with geocode-api data.' . $suggestions->get_error_message();
            }
        ...
        if ( $raw = get_post_meta( $post_id, 'raw', true ) ) {
            ...
            switch ( $political_level ) {
                ...
                case 'street_address':
                    break;
                default:
                    return new WP_Error( __METHOD__, 'Undefined geocode-api data. Regeocode location.' );
                    break;
            }
        } else {
            echo __( 'First geocode this location using the geocode-api metabox, and then we can offer suggestions to connect it with the network mapping system.' );
        }
    }
